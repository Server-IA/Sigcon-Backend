package com.sigcon.backend.third_parties.interfaces;
...
import com.sigcon.backend.third_parties.domain.service.ThirdPartyService;
...
@Tag(name = "Terceros", description = "Endpoints para gestion de terceros")
...
public class ThirdPartyController {
    ...
    @Operation(
            summary = "Carga masiva de terceros",
            description = "RF07 - Importa terceros desde archivo CSV/XLSX enviado en base64. " +
                    "No permite estado BLOQUEADO en cargue masivo.",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    required = true,
                    description = "Nombre del archivo, contenido base64, delimitador (si es CSV) y overwrite S/N.",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = BulkThirdPartyUploadRequest.class),
    ...
    public ResponseEntity<?> bulkStore(@Valid @RequestBody BulkThirdPartyUploadRequest request, BindingResult bindingResult) {
        return thirdPartyService.bulkStore(request, bindingResult);
    }

    @PostMapping("/search")
    @Operation(
            summary = "Consultar terceros",
            description = "RF01 - Consulta paginada con filtros avanzados (DataTable).",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    required = false,
    ...
    @Operation(
            ...
            description = "RF03 - Actualiza la informacion de un tercero existente.",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    required = true,
                    content = @Content(
                            mediaType = "application/json",
    ...
    @Operation(
            summary = "Gestionar roles y estado",
            description = "RF04 - Actualiza los roles y estado del tercero. " +
                    "Si el estado es BLOQUEADO, requiere blockingReason >= 20.",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    required = true,
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = UpdateThirdPartyRolesStatusRequest.class),
