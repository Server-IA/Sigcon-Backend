...
public interface VoucherRepository extends JpaRepository<VouchersEntity, Long>, JpaSpecificationExecutor<VouchersEntity> {
    ...
    // Multi-tenant: todas las queries nativas incluyen v.company_id = :companyId
    // para evitar cross-tenant leaks (las queries nativas no respetan @Filter).
    ...
    """, nativeQuery = true)
    BigInteger findTopByVoucherTypeIdAndDeletedAtIsNullOrderByNumberDesc(
            @Param("voucherTypeId") Long voucherTypeId,
            @Param("companyId") Long companyId);

    @Query(value = """
        SELECT v.* FROM vouchers v
        WHERE v.asset_id = :assetId AND v.company_id = :companyId AND v.deleted_at IS NULL
    """, nativeQuery = true)
    List<VouchersEntity> findAllByAssetIdAndDeletedAtIsNull(
            @Param("assetId") Long assetId,
            @Param("companyId") Long companyId);

    @Query(value = """
        SELECT SUM(v.amount) FROM vouchers v
        WHERE v.bank_account_id = :bankAccountId AND v.company_id = :companyId AND v.deleted_at IS NULL
    """, nativeQuery = true)
    BigDecimal sumVouchersByBankAccountId(
    ...
    @Query(value = """
        ...
        WHERE c.id = :checkId AND v.company_id = :companyId AND v.deleted_at IS NULL
    """, nativeQuery = true)
    BigDecimal sumVouchersByCheckbookId(
