...
import com.sigcon.backend.assets.application.CreateAssetsDTO;
import com.sigcon.backend.assets.application.UpdateAssetsDTO;
import com.sigcon.backend.assets.application.ViewAssetsDTO;
import com.sigcon.backend.assets.domain.service.AssetsService;
...
@Tag(name = "Assets", description = "Endpoints para gestionar activos (CRU)")
public class AssetsController {
    ...
    @Operation(
            ...
            description = "ACT-RF-01: crea un activo con validaciones contables y de terceros."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Activo registrado correctamente",
    ...
    public ResponseEntity<?> store(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    required = true,
                    description = "Datos requeridos para crear el activo."
            )
            @Valid @RequestBody CreateAssetsDTO request,
    ...
    @Operation(
            summary = "Consultar activos",
            description = "Consulta activos en formato DataTable."
    )
    @ApiResponses(value = {
    ...
    @Operation(
            ...
            description = "Obtiene la informacion de un activo especifico."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Detalle obtenido correctamente",
                    content = @Content(schema = @Schema(implementation = Object.class))),
            @ApiResponse(responseCode = "400", description = "Activo no encontrado",
                    content = @Content(schema = @Schema(implementation = Object.class)))
    })
    ...
    @Operation(
            summary = "Editar activo",
            description = "ACT-RF-09: actualiza un activo existente."
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> update(
            @Parameter(description = "ID del activo a actualizar", example = "1")
            @PathVariable Long id,
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    required = true,
                    description = "Datos requeridos para actualizar el activo."
            )
            @Valid @RequestBody UpdateAssetsDTO request,
            @Parameter(hidden = true)
            BindingResult bindingResult
    ) {
        if (bindingResult.hasErrors()) {
