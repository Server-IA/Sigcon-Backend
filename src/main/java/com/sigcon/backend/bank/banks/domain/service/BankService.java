...
import com.sigcon.backend.bank.banks.application.BankDTO;
...
public class BankService {
    ...
    private final DataTableSpecificationBuilder<Bank> dataTableSpecificationBuilder =
   ...
   public ResponseEntity<?> create(BankDTO request, BindingResult bindingResult) {
    ...
    public ResponseEntity<?> update(Long id, BankDTO request, BindingResult bindingResult) {
        ...
        if (request.getCode() != null)             bank.setCode(request.getCode().trim());
        if (request.getName() != null)             bank.setName(request.getName().trim());
        if (request.getNameShort() != null)        bank.setNameShort(request.getNameShort().trim());
        if (request.getTypeBank() != null)         bank.setTypeBank(request.getTypeBank());
        if (request.getNit() != null)              bank.setNit(request.getNit().trim());
        if (request.getSwift() != null)            bank.setSwift(request.getSwift().trim());
        if (request.getCodeAch() != null)          bank.setCodeAch(request.getCodeAch());
        if (request.getUrlWebservice() != null)    bank.setUrlWebservice(request.getUrlWebservice());
        if (request.getConciliationDays() != null) bank.setConciliationDays(request.getConciliationDays());
        if (request.getPhone() != null)            bank.setPhone(request.getPhone());
        if (request.getStatus() != null)           bank.setStatus(request.getStatus());
        if (request.getFormatExtract() != null)    bank.setFormatExtract(request.getFormatExtract());
    ...
    private Country resolveCountry(Long countryId) {
        ...
        return countryRepository.findById(countryId)
                .orElseThrow(() ->
                        new IllegalArgumentException("El país no existe."));
    }
