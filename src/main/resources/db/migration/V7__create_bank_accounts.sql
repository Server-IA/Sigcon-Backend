...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_bank_accounts_chart_of_account') THEN
        ALTER TABLE bank_accounts ADD CONSTRAINT fk_bank_accounts_chart_of_account FOREIGN KEY (chart_of_account_id) REFERENCES cfg_chart_of_accounts(id);
    END IF;
