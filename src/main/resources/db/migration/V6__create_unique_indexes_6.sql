...
CREATE INDEX IF NOT EXISTS idx_checks_beneficiary
ON checks (beneficiary)
WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_checks_checkbook
ON checks (checkbooks_id)
WHERE deleted_at IS NULL;
